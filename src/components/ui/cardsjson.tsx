                                        ...
                                        <div className="card bg-surface-100 hover:bg-surface-200 group flex h-full w-full flex-col rounded-xl border px-4 py-4 shadow transition-all hover:shadow-lg">
                                            <div className="flex flex-col">
                                                ...
                                                <div className="flex justify-end">
                                                    {item.category ? <Badge variant="secondary" className="badge badge-category text-xs mt-2">{item.category}</Badge> : null}

                                                    {item.city ? <Badge variant="outline" className="badge badge-city text-xs mt-2 ml-2">{item.city}</Badge> :
                                                    item.country ? <Badge variant="outline" className="badge badge-country text-xs mt-2 ml-2">{item.country}</Badge> : 
                                                    item.continent ? <Badge variant="outline" className="badge badge-continent text-xs mt-2 ml-2">{item.continent}</Badge> : null}
                                                </div>
                                            </div>
                                        </div>
                                    </a>
